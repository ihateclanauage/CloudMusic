...
Page({
  ...
  toSongDetail(e) {
    // 把搜索结果做成歌单
    var searchlist = this.data.searchResult
    var songlist = []
    for (var i = 0; i < searchlist[0].length - 1; i++){
      songlist[i] = searchlist[0][i].id
    }

    let contacts = {
      id: e.currentTarget.dataset.musicid,
      Idlist: songlist,
    }
    // 先对数据进行JSON
    ...
    // wx.navigateTo({
    //   url: '/pages/play/play?song=' + e.currentTarget.dataset.musicid + '&type=searchList',
    // });
  },
